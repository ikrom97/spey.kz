<ul class="results">
    @foreach ($result->products as $product)
        <li class="results-item">
            <a class="results-link" href="{{route('products.read', $product->id)}}">
                ...
                <span class="results-breadcrumbs">{{__('Home')}}/{{__('Products')}}</span>
    ...
    @foreach ($result->news as $news)
        <li class="results-item">
            <a class="results-link" href="{{route('news.read', $news->id)}}">
                ...
                <span class="results-breadcrumbs">{{__('Home')}}/{{__('Industry news')}}</span>
                <span class="results-more-icon">
                    <svg width="21" height="21" xmlns="http://www.w3.org/2000/svg"><g fill="none" fill-rule="evenodd" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M13.5 6.497l4 4.002-4 4.001M4.5 10.5h13"/></g></svg>
                </span>
            </a>
    ...
    @endforeach
    @if ($result->products->count() == 0 && $result->news->count() == 0)
